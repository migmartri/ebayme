class <%= model_controller_class_name %>Controller < ApplicationController
  ...
  def update
    ...
    if @<%= file_name %>.update_attributes(params[:<%= file_name %>])
      flash[:notice] = "<%= class_name %> updated"
  ...
  def destroy
    @<%= file_name %> = <%= class_name %>.find(params[:id])
    if @<%= file_name %>.update_attribute(:enabled, false)
      flash[:notice] = "<%= class_name %> disabled"
    else
      flash[:error] = "There was a problem disabling this <%= file_name %>."
    end
  ...
  def enable
    @<%= file_name %> = <%= class_name %>.find(params[:id])
    if @<%= file_name %>.update_attribute(:enabled, true)
      flash[:notice] = "<%= class_name %> enabled"
    else
      flash[:error] = "There was a problem enabling this <%= file_name %>."
    end
    redirect_to :action => 'index'
  end
